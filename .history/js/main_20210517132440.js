...
const impulses = ['bathroom.wav', 'big_closet.wav'];
var segments = [
	{
...
for (var i = 0; i < segments.length; i++) {
	(function(segment) {
		segment.audio.on('play', function() {
            $(segment.playButton).find('i').removeClass('ion-ios-play').addClass('ion-ios-pause');
		...
		segment.audio.on('stop', function() {
            if ($(segment.playButton).find('ios-ion-pause')) {
                $(segment.playButton).find('i').removeClass('ion-ios-pause').addClass('ion-ios-play');
            }
...
//		}
		...
		if (!segment.effects || !segment.effects.length || true)
			return;
